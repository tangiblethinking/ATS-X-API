...
import { FileCode2, Play, RotateCcw, Moon, Sun, X, User } from "lucide-react";
import { PdfConvertButton } from "@/components/pdf-convert-button";
import { ApiKeyDialog } from "@/components/api-key-dialog";
import { WelcomeModal } from "@/components/welcome-modal";
import { ApiKeyWizard } from "@/components/api-key-wizard";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    applyTheme(t);
    setHydrated(true);
    if (!readStoredApiKey()) setShowWelcome(true);
  }, []);
  ...
  function handleResetApp() {
    ...
    setShowWelcome(true);
    toast.success("App reset. API key cleared.");
  }
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button type="button" variant="ghost" size="sm" className="h-11 w-11 p-0" aria-label="User menu">
                    <User className="size-4" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onSelect={handleResetApp}>Reset app</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          </div>
